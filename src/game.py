...
from src.input_map import InputMapper
...
class Game:
    ...
    def __init__(self) -> None:
        ...
        self.input_mapper = InputMapper({
            ...
            Action.JUMP: {pygame.K_w, pygame.K_SPACE},
            Action.GROUND_POUND: {pygame.K_g}})

        self.ground_pound_cooldown = 1.0  # seconds
        self.ground_pound_cooldown_left = 0.0
        #########################################################################
    ...
    def handle_event(self, event: pygame.event.Event) -> None:
        ...
        if event.key == pygame.K_4:
            self.preset_idx = 3  # slow_floaty"

        if event.key == pygame.K_5:
            self.preset_idx = 4  # fast

        if self.state != "play":
            return

        # Discrete actions TEST, 
        if self.input_mapper.is_triggered(event, Action.DASH):
            self._try_dash()
            return

        if self.platformer_mode and self.input_mapper.is_triggered(event, Action.JUMP):
            self.jump_requested = True

        # Ground Pound (discrete action)
        if self.input_mapper.is_triggered(event, Action.GROUND_POUND):
            self._try_ground_pound()
    ...
    def _read_direction(self) -> pygame.Vector2:
        ...
        if self.input_mapper.is_pressed(Action.MOVE_LEFT):
            x -= 1
        if self.input_mapper.is_pressed(Action.MOVE_RIGHT):
            x += 1
        if self.input_mapper.is_pressed(Action.MOVE_UP):
            y -= 1
        if self.input_mapper.is_pressed(Action.MOVE_DOWN):
            y += 1
    ...
    def update(self, dt: float) -> None:
        ...
        if self.ground_pound_cooldown_left > 0:
            self.ground_pound_cooldown_left = max(0.0, self.ground_pound_cooldown_left - dt)
    ...
    def _draw_debug(self) -> None:
        ...
        if self.jump_requested:
            lines.append("Jump triggered!")

        if self.ground_pound_cooldown_left > 0:
            lines.append(f"Ground Pound cooldown: {self.ground_pound_cooldown_left:0.2f}s")
        else:
            # Show ready state
            lines.append("Ground Pound READY")
    ...
    def draw(self) -> None:
        ...
        if self.state == "title":
            self._draw_center_message(
                "Week 3 - A3",
                "Space: start   Shift: dash   1/2/3/4/5: feel    TAB: Boundary mode   C: scheme   R: Reset   P: mode   F1: debug",
            )
    def _try_ground_pound(self) -> None:
        # Constraint 1: must be airborne
        if self.on_ground:
            return

        # Constraint 2: cooldown
        if self.ground_pound_cooldown_left > 0:
            return

        # Apply ground pound effect (placeholder)
        # For example: instantly increase downward velocity
        self.player_vel.y += 1200.0  # tune later

        # Reset cooldown
        self.ground_pound_cooldown_left = self.ground_pound_cooldown

        # Optional: debug print
        if self.debug:
            print("Ground Pound triggered!")
